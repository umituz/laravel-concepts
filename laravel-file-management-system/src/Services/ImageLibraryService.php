...
## --------------------------------------------------------
		...
		#
	{
		...
		$this->fileExtension = GalleryHelper::fixStrToLower(strrchr($fileName, '.'));
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$cropPos = 'm';
		if (is_array($option) && GalleryHelper::fixStrToLower($option[0]) == 'crop') {
...
## --------------------------------------------------------
	...
	private function getCropPlacing($optimalWidth, $optimalHeight, $newWidth, $newHeight, $pos = 'm')
		...
		#
	{
		$pos = GalleryHelper::fixStrToLower($pos);
...
## --------------------------------------------------------
	...
	{
		if (is_array($option)) {
			if (GalleryHelper::fixStrToLower($option[0]) == 'crop' && count($option) == 2) {
				return 'crop';
			} else {
		...
		if (is_string($option)) {
			return GalleryHelper::fixStrToLower($option);
		}
		...
		#
	{
		if ($this->imageResized) {
			...
			if (is_string($value)) {
				...
				$value = GalleryHelper::fixStrToLower($value);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		$isTransparent = false;
		if (!is_array($bgColor)) {
			if (GalleryHelper::fixStrToLower($bgColor) == 'transparent') {
				$isTransparent = true;
			}
		}
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	public function addCaptionBox($side = 'b', $thickness = 50, $padding = 0, $bgColor = '#000', $transaprencyAmount = 30)
		...
		#
	{
		$side = GalleryHelper::fixStrToLower($side);
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
		...
		#
	{
		...
		if (GalleryHelper::fixStrToLower(strrchr($watermarkImage, '.')) == '.png') {
...
## --------------------------------------------------------
		...
		#
	{
		...
		if (is_array($value)) {
			...
			if (GalleryHelper::fixStrToLower($value) == 'transparent') {
