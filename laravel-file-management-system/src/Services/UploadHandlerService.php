	...
	public function onUploadEnd($res)
	{
		...
		if ($is_img) {
			if (isset($this->options['config']['image_watermark']) && $this->options['config']['image_watermark']) {
				...
				$magicianObj = new ImageLibraryService($targetFile);
			...
			$thumbResult = GalleryHelper::createImg($targetFile, $targetFileThumb, 122, 91);
			...
			if ($thumbResult !== true) {
				...
				if (!$this->options['ftp'] && !GalleryHelper::newThumbnailsCreation($targetPath,
						$targetFile,
						$_FILES['files']['name'][0],
						$this->options['config']['current_path']
					)) {
					$res['files'][0]->error = trans("Not enough Memory");
				} else {
					...
					if ($this->options['config']['image_resizing']) {
						...
						GalleryHelper::createImg($targetFile, $targetFile, $this->options['config']['image_resizing_width'], $this->options['config']['image_resizing_height'], $this->options['config']['image_resizing_mode']);
					}
					...
					if ($resize) {
						GalleryHelper::createImg($targetFile, $targetFile, $srcWidth, $srcHeight, $this->options['config']['image_max_mode']);
					}
				}
			}
		}
